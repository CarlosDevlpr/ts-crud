...
import { flatten, forEach, isNil, keys, map, mergeAll, pick, pipe } from "ramda"
...
export const ResponseController = (request: Request, response: Response) => {
    ...
    const mergeAllDeep = (schema: any) => (obj: any) => {
        ...
        forEach((k: any) => {
            ...
            if (isObjArray && schema.fields[k].type === 'array') {
                return obj[k]
            } else if (isObjArray && schema.fields[k].type !== 'array') {
